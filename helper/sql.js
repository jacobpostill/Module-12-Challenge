const department = `SELECT name AS Name, department_id AS 'Department ID'
FROM department;`

const roles = `SELECT roles.role_id AS ID, roles.title AS Title, department.name AS Department, roles.salary as Salary
FROM department
INNER JOIN roles ON department.department_id = roles.department_id;`

const employees = `SELECT e.employees_id as "ID", e.first_name as "First Name", e.last_name as "Last Name", roles.title as "Job Title", department.name AS "Department", roles.salary as 'Salary',CONCAT(m.first_name, ' ', m.last_name) AS manager
FROM   employees e
